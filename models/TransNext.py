...
class FreqDomainGLU(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        hidden_features = int(2 * hidden_features / 3)  # 调整隐藏层特征数

        self.fc1 = nn.Linear(in_features, hidden_features * 2)  # 乘以2为GLU准备
        self.act = act_layer()  # 激活函数
        self.fc2 = nn.Linear(hidden_features, out_features)  # 输出层线性变换
        self.drop = nn.Dropout(drop)  # Dropout减少过拟合

        # 为每个输入通道初始化频域卷积的权重
        self.freq_conv_weight = nn.Parameter(torch.randn(1, 1, 1, 1))
    ...
    def forward(self, x, H, W):
        ...
        # 在频域应用卷积操作
        x_freq_processed = self.dwconv_in_freq(x_freq, C)

        # 逆傅里叶变换回时域
        x_processed = torch.fft.irfft2(x_freq_processed, s=(H, W), dim=(2, 3), norm='ortho')

        # 将张量重塑回原始形状
        x_processed = x_processed.permute(0, 2, 3, 1).contiguous().view(B, N, -1)

        # 应用门控机制
        x = self.act(x_processed) * v
        x = self.drop(x)
        x = self.fc2(x)
        x = self.drop(x)
        return x

    def dwconv_in_freq(self, x_freq, channel_size):
        ...
        x_freq_real = F.conv2d(x_freq.real, weight, self.freq_conv_bias, groups=groups)
        x_freq_imag = F.conv2d(x_freq.imag, weight, self.freq_conv_bias, groups=groups)
...
class CrossAggregatedAttention(nn.Module):
    def __init__(self, dim, input_resolution, num_heads=8, window_size=3, qkv_bias=True,
                 attn_drop=0., proj_drop=0., sr_ratio=1):
        super().__init__()
        assert dim % num_heads == 0, f"dim {dim} should be divided by num_heads {num_heads}."
        ...
        # Cross-Attention
        self.cross_attn = nn.MultiheadAttention(dim, 1, dropout=attn_drop, bias=qkv_bias)
    ...
    def forward(self, x, H, W, relative_pos_index, relative_coords_table, seq_length_scale, padding_mask):
        ...
        # Cross-Attention
        x_global = x.permute(1, 0, 2)
        x_local = x.permute(1, 0, 2)
        #x_cross, _ = self.cross_attn(x_global, x_local, x_local)
        x_cross, _ = self.cross_attn(x_local, x_global, x_global)
        x_cross = x_cross.permute(1, 0, 2)
        x = x + x_cross
    ...
    def __init__(self, dim, num_heads, input_resolution, window_size=3, mlp_ratio=4.,
        ...
        if sr_ratio == 1:
            self.attn = Attention(
                ...
                attn_drop=attn_drop,
                proj_drop=drop)
            # self.attn = CrossAggregatedAttention(
            #     dim,
            #     input_resolution,
        ...
        self.mlp = FreqDomainGLU(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class TransNeXt(nn.Module):
    ...
    '''
    def __init__(self, img_size=224, pretrain_size=None, window_size=[3, 3, 3, 3],
...
class transnext_tiny(TransNeXt):
    def __init__(self, **kwargs):
        super().__init__(window_size=[3, 3, 3, 3],
                         patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[3, 6, 12, 24],
...
class transnext_base(TransNeXt):
    def __init__(self, **kwargs):
        super().__init__(window_size=[3, 3, 3, 3],
                         patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[4, 8, 16, 32],
                         mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
class transnext_base_decoder(nn.Module):
    def __init__(self, input_nc=3, output_nc=2, embed_dim=256, embed_dims=[64, 128, 320, 512],
        ...
        super(transnext_base_decoder, self).__init__()

        self.embedding_dim = embed_dim
        self.embed_dims = embed_dims

        self.Tenc_x2 = transnext_base()
...
if __name__ == '__main__':
    ...
    net = transnext_base(retrained=None).cuda()
    print(net)
    for i in range(4):
        print(net(x1)[i].shape)
