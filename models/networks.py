...
from models.FTAN import ChangeNext_diff,ChangeNext_decoder,ChangeNextV1,ChangeFormer_decoder,ChangeNextV3,ChangeNextV2
...
from models.NewCD import NewCD
from models.TransNext import transnext_base_decoder
from models.TransNext import *
###############################################################################
# Helper Functions
###############################################################################
...
def define_G(args, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif args.net_G == 'ChangeNext_b':
        net = ChangeNext_diff(embed_dim=args.embed_dim, decoder_softmax=False, embed_dims=[64, 128, 320, 512], mlp_ratios=[8, 8, 4, 4], drop_rate=0.1,
                             drop_path_rate=0.2, depths=[3,3,12,3], num_stages=4)
    elif args.net_G == 'ChangeNext_L':
        net = ChangeNext_diff(embed_dim=args.embed_dim, decoder_softmax=False, embed_dims=[64, 128, 320, 512], mlp_ratios=[8, 8, 4, 4], drop_rate=0.2,
                             drop_path_rate=0.3, depths=[3, 5, 27, 3], num_stages=4)

    elif args.net_G == 'ChangeNextV1':
        net = ChangeNextV1(embed_dim=args.embed_dim,embed_dims=[64, 128, 320, 512],mlp_ratios=[8, 8, 4, 4], drop_rate=0.,
                 drop_path_rate=0.1, depths=[3, 5, 12, 3], num_stages=4)

    elif args.net_G == 'ChangeNextV2':
        net = ChangeNextV2(embed_dim=args.embed_dim,embed_dims=[64, 128, 320, 512],mlp_ratios=[8, 8, 4, 4], drop_rate=0.1,
                 drop_path_rate=0.1, depths=[3, 3, 12, 3], num_stages=4)

    elif args.net_G == 'ChangeNextV3':
        net = ChangeNextV3(embed_dim=args.embed_dim,embed_dims=[64, 128, 320, 512],mlp_ratios=[8, 8, 4, 4], drop_rate=0.1,
                 drop_path_rate=0.2, depths=[3, 3, 12, 3], num_stages=4)
    # net = ChangeNextV3(embed_dim=args.embed_dim,embed_dims=[64, 128, 320, 512],mlp_ratios=[8, 8, 4, 4], drop_rate=0.,
    #              drop_path_rate=0.1, depths=[3, 3, 12, 3], num_stages=4)

    elif args.net_G == 'ChangeFormer_Decoder':
        net = ChangeFormer_decoder(embed_dim=args.embed_dim)

    #net = ChangeNext_decoder(embed_dim=args.embed_dim,embed_dims=[64, 128, 320, 512],mlp_ratios=[8, 8, 4, 4], drop_rate=0.1,
     #            drop_path_rate=0.2, depths=[3, 3, 12, 3], num_stages=4)

    elif args.net_G == 'ChangeNext_Decoder':
        net = ChangeNext_decoder(embed_dim=args.embed_dim,embed_dims=[64, 128, 320, 512],mlp_ratios=[8, 8, 4, 4], drop_rate=0.2,
                 drop_path_rate=0.5, depths=[3, 3, 12, 3], num_stages=4)

    elif args.net_G == 'NewCD_Decoder':
        net = NewCD(embed_dim=args.embed_dim)
    
    elif args.net_G == 'TransNext':
        net= transnext_base_decoder(output_nc=2, embed_dim=args.embed_dim, embed_dims=[64, 128, 320, 512])
