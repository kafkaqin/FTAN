...
from models.losses import cross_entropy,ohem_cross_entropy
...
class CDTrainer():
    ...
    def _backward_G(self):
        ...
        ohem_sampler = OHEMPixelSampler(context=self.net_G, thresh=0.7, min_kept=100000, ignore_index=255)
        if self.multi_scale_train == "True":
            ...
            for pred in self.G_pred:
                if pred.size(2) != gt.size(2):
                    ...
                    seg_weight = ohem_sampler.sample(pred, F.interpolate(gt, size=pred.size(2), mode="nearest"))
